...
export class SucursalesService {
        ...
        throw new HttpException(
          {
            ...
            message: 'Error consultando las sucursales',
          },
  ...
  async createSucursal(
    ...
    const data: Prisma.SucursalCreateInput = {
      nombre,
      telefono,
      direccion,
      creadoPor,
    ...
    try {
      ...
      if (e.response === HttpStatus.INTERNAL_SERVER_ERROR) {
        throw new HttpException(
          {
            status: HttpStatus.INTERNAL_SERVER_ERROR,
            message: 'Error creando la sucursal',
          },
          HttpStatus.INTERNAL_SERVER_ERROR
        );
      } else {
  ...
  async updateSucursal(params: {
    ...
    const { where } = params;
    const { data } = params;
    let sucursalUpdate: Prisma.SucursalUpdateInput = {
      ...(data as Prisma.SucursalUpdateInput),
    };
    ...
    try {
      ...
      if (e.response === HttpStatus.INTERNAL_SERVER_ERROR) {
        throw new HttpException(
          {
            status: HttpStatus.INTERNAL_SERVER_ERROR,
            message: 'Error actualizando la sucursal',
          },
          HttpStatus.INTERNAL_SERVER_ERROR
        );
      } else {
    ...
    try {
      ...
      if (!response || response === HttpStatus.INTERNAL_SERVER_ERROR) {
        throw new HttpException(
          {
            status: HttpStatus.INTERNAL_SERVER_ERROR,
            message: 'Error inhabilitando la sucursal',
          },
          HttpStatus.INTERNAL_SERVER_ERROR
        );
      } else {
